...
class SOAP_Type_duration
{
    ...
    static function unix_to_duration($seconds)
    {
        return SOAP_Type_duration::getduration($seconds);
    }

    static function mod($a, $b, &$d, &$r)
    {
        $d = floor($a / $b);
        $r = $a % $b;
    }

    static function getduration($seconds)
    {
        $neg = '';
        if ($seconds < 0) {
            $neg = '-';
    ...
    static function mkduration($n, $Y, $Mo, $D, $H, $Mi, $S)
    {
        $_mi = 60;
        $_h = $_mi * 60;
        $_d = $_h * 24;
    ...
    static function duration_to_unix($duration)
    {
